var FantasyExplorer = function () {
    ...
    var _apiUrl = {
        "list": $('base').prop('href') + "api/storage/list",
        "mkdir": $('base').prop('href') + "api/storage/mkdir",
        "move": $('base').prop('href') + "api/storage/move",
        "rmdir": $('base').prop('href') + "api/storage/rmdir",
        "unlink": $('base').prop('href') + "api/storage/unlink"
    };
    ...
    var _fileTableOptions = {
        ...
        'rowCallback': function (row, data) {
            ...
            if (data.ext === 'jpg' || data.ext === 'png' || data.ext === 'gif') {
                $(row).data('content', '<img style="width:100%;height:100%" src="imagefly/w300-h200-c/uploads/' + data.filepath + '" />');
                $(row).data('placement', 'left');
                $(row).popover({
                    'trigger': 'hover',
                    'html': true
                });
        ...
        setTimeout(function () {
            ...
            $("#file_upload").uploadify({
                ...
                'onUploadProgress': function (file, bytesUploaded, bytesTotal, totalBytesUploaded, totalBytesTotal) {
                    ...
                    var value = Math.round((bytesUploaded / bytesTotal) * 100);
                    _uploadProgress.find('div.uploading')
                            .find('.progress-bar')
                            .css('width', value + '%')
