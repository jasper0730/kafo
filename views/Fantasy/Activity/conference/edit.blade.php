...
@section('title',"說法會管理頁面管理頁面修改 "."-".$ProjectName)
@section('css')
...
<a href="" id="toFileManager"></a>
		...
		<!-- BEGIN PAGE HEADER-->
		<h3 class="page-title">
			說法會管理頁面 <small>News Manager</small> </h3>
		<div class="page-bar">
			<ul class="page-breadcrumb">
				...
				<li>
					<a href="{{ ItemMaker::url('Fantasy/'.$routePreFix)}}">說法會管理頁面</a> <i class="fa fa-angle-right"></i>
				</li>
				<li>
					說法會管理頁面修改 </li>
			</ul>
                                ...
                                <div class="portlet-title portlet-title2">
                                    <div class="caption">
                                        ...
                                        <span class="caption-subject font-dark sbold uppercase">
											{{ ( !empty( $data['title'] ) )? $data['title'] : '' }} - 說法會管理頁面 </span>
                                    </div>
                                        ...
                                        <div class="tab-content">
                                            <div class="tab-pane active form-row-seperated" id="tab_general">
                                                <div class="form-body form_news_pad">
													...
													{{ItemMaker::idInput([
														"inputName" => "conference[id]",
														"value" => ( !empty($data["id"]) )? $data["id"] : ""
													])}}
													{{ItemMaker::radio_btn([
														"labelText" => "是否啟用",
														"inputName" => "conference[is_visible]",
														"helpText" =>"是否啟用",
													...
													{{ItemMaker::textInput([
														"labelText" => "排序",
														"inputName" => "conference[rank]",
														"value" => ( !empty($data["rank"]) )? $data["rank"] : ""
													])}}
													{{ItemMaker::textInput([
														"labelText" => "標題",
														"inputName" => "conference[title]",
														"value" => ( !empty($data["title"]) )? $data["title"] : ""
													])}}
													{{ItemMaker::textInput([
														"labelText" => "年份",
														"inputName" => "conference[year]",
														"value" => ( !empty($data["year"]) )? $data["year"] : ""
													])}}
													{{ItemMaker::filePicker([
														"labelText" => "選擇檔案",
														"inputName" => "conference[file]",
