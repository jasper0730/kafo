...
<div class="wrapper page-revenue">
  ...
  <section id="revenue" class="revenue-wrap">
    <a href="{{ ItemMaker::url("Policy/orgnization") }}#revenue">
      <h2 class="move title">
    ...
    <div class="inner">
      ...
      <p class="management-set">經理人之設置</p>

      <!--中間內容-->
      <section class="director-info management-group">
        @foreach($orgnization as $value)
        <article class="director-article management-article clearfix">
          <h1 class="director-name">{{ $value->name }}</h1>
          <p class="management-business">{{ $value->position }}</p>
        </article>
        @endforeach
      ...
      <table class="table-type1 shareholder-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            @else
              <td class="title_1 org_download" data-title="檔案下載"></span></a></td>
            @endif
          </tr>
          @endforeach
        </tbody>
